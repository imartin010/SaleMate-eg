-- ================================================
-- STEP 1: Reset all projects to 0 leads and 0 price
-- ================================================
...
-- ================================================
-- STEP 2: Create auto-counting system for leads
-- ================================================
...
-- ================================================
-- STEP 3: Helper functions for lead management
-- ================================================
...
BEGIN
    ...
    -- Update project CPL price first
    ...
    -- The available_leads count will be automatically updated by the trigger
    
    -- Create batch record if batch_name is provided and lead_batches table exists
    BEGIN
        ...
        END IF;
    EXCEPTION WHEN OTHERS THEN
        -- Ignore if lead_batches table doesn't exist
        NULL;
    END;
    ...
    RETURN jsonb_build_object(
        ...
        'message', 'Leads uploaded successfully. Available leads count updated automatically.'
    );
END;
$$ LANGUAGE plpgsql SECURITY DEFINER;
...
-- Query to check projects with 0 leads and 0 price
-- SELECT name, developer, available_leads, price_per_lead FROM projects ORDER BY name;

-- Query to check leads count per project
-- SELECT p.name, p.developer, COUNT(l.id) as actual_leads, p.available_leads 
-- FROM projects p 
-- LEFT JOIN leads l ON p.id = l.project_id AND (l.is_sold IS FALSE OR l.is_sold IS NULL)
-- GROUP BY p.id, p.name, p.developer, p.available_leads
-- ORDER BY p.name;
