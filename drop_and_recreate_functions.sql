-- DROP AND RECREATE PURCHASE FUNCTIONS
-- This fixes the parameter name conflict

-- 1) Drop existing functions first
DROP FUNCTION IF EXISTS public.rpc_get_project_availability(uuid);
DROP FUNCTION IF EXISTS public.rpc_start_order(text, uuid, integer, uuid);
DROP FUNCTION IF EXISTS public.rpc_confirm_order(uuid, text, text);

-- 2) Create rpc_get_project_availability function
...
-- 3) Create rpc_start_order function
...
-- 4) Create rpc_confirm_order function
