-- COMPLETE AUTHENTICATION SYSTEM - FINAL SQL
-- Run this in Supabase SQL Editor to fix all auth issues
...
-- 11) Execute one-time backfill
SELECT public.backfill_profiles() as profiles_created;

-- 12) Create default admin user if none exists
...
-- 13) Update any existing user with admin email to admin role
UPDATE public.profiles 
SET role = 'admin'::user_role, updated_at = NOW()
WHERE email ILIKE '%admin%' OR email = 'themartining@gmail.com';
...
-- 15) Show all profiles for verification
SELECT 
    id,
    email,
    name,
