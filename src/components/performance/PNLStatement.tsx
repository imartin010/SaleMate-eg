...
export const PNLStatement: React.FC<PNLStatementProps> = ({
  ...
  return (
    <div className="space-y-4">
      {/* Header */}
      <div className="bg-gradient-to-br from-slate-50 to-gray-50 rounded-xl shadow-md border-2 border-gray-300 p-6">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center space-x-3">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600 mt-1 font-medium">{franchise.name}</p>
            </div>
          </div>
          <div className={`px-5 py-3 rounded-xl border-2 shadow-lg ${
            isProfitable 
              ...
              return (
                ...
                >
                  <div className="flex items-center space-x-2">
                    ...
                    <span className={`${isTotal ? 'text-base font-bold' : isSubtotal ? 'text-sm font-bold' : 'text-sm'}`}>
                      {row.label}
                    </span>
                  </div>
                  <span
      ...
      <div className="bg-gradient-to-br from-slate-50 to-gray-50 rounded-xl shadow-md border-2 border-gray-300 overflow-hidden">
        <div className="p-6">
          <div className="flex items-center space-x-3 mb-4">
            ...
            <h3 className="text-xl font-bold text-gray-900">Monthly P&L Breakdown (Last 12 Months)</h3>
          </div>
            ...
            <table className="w-full">
              <thead>
                <tr className="bg-gradient-to-r from-gray-100 to-slate-100 border-b-2 border-gray-300">
                  <th className="text-left py-4 px-4 text-sm font-bold text-gray-800 uppercase tracking-wide">Month</th>
                  <th className="text-right py-4 px-4 text-sm font-bold text-green-700 uppercase tracking-wide">Revenue</th>
                  <th className="text-right py-4 px-4 text-sm font-bold text-orange-700 uppercase tracking-wide">Expenses</th>
                  <th className="text-right py-4 px-4 text-sm font-bold text-indigo-700 uppercase tracking-wide">Profit/(Loss)</th>
                </tr>
              </thead>
              <tbody>
                {monthlyPNL.map((month, index) => (
                  ...
                  >
                    <td className="py-3 px-4 text-sm font-bold text-gray-800">{month.monthLabel}</td>
                    <td className="py-3 px-4 text-sm text-right font-mono font-bold text-green-600">
                      {formatCurrency(month.revenue)}
                    </td>
                    <td className="py-3 px-4 text-sm text-right font-mono font-bold text-orange-600">
              ...
              <tfoot>
                <tr className={`border-t-2 font-bold shadow-md ${
                  ...
                  <td className="py-4 px-4 text-sm text-gray-900 font-bold uppercase">Total</td>
                  <td className="py-4 px-4 text-sm text-right font-mono text-green-700 font-bold">
                    {formatCurrency(monthlyPNL.reduce((sum, m) => sum + m.revenue, 0))}
                  </td>
                  <td className="py-4 px-4 text-sm text-right font-mono text-orange-700 font-bold">
                    {formatCurrency(monthlyPNL.reduce((sum, m) => sum + m.expenses, 0))}
                  </td>
                  <td className={`py-4 px-4 text-base text-right font-mono font-bold ${
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <div className="bg-gradient-to-br from-green-50 to-emerald-50 rounded-xl p-6 border-2 border-green-300 shadow-md hover:shadow-lg transition-shadow">
          <div className="flex items-center space-x-2 mb-3">
            ...
            <p className="text-sm font-bold text-gray-700">Gross Revenue</p>
          </div>
          <p className="text-3xl font-bold text-green-700">{formatCurrency(analytics.gross_revenue)}</p>
          <p className="text-xs text-green-600 mt-2 font-semibold">Total commissions earned</p>
        </div>

        <div className="bg-gradient-to-br from-orange-50 to-amber-50 rounded-xl p-6 border-2 border-orange-300 shadow-md hover:shadow-lg transition-shadow">
          <div className="flex items-center space-x-2 mb-3">
            <div className="p-2 bg-white rounded-lg shadow-sm">
          ...
          <p className="text-xs text-orange-600 mt-2 font-semibold">Fixed + Variable + Cuts + Taxes</p>
        </div>

        <div className={`rounded-xl p-6 border-2 shadow-md hover:shadow-lg transition-shadow ${
          isProfitable
          ...
          <div className="flex items-center space-x-2 mb-3">
            ...
            <p className={`text-sm font-bold ${isProfitable ? 'text-emerald-700' : 'text-red-700'}`}>
              Net {isProfitable ? 'Profit' : 'Loss'}
            </p>
          </div>
          ...
          <p className={`text-xs mt-2 font-semibold ${isProfitable ? 'text-emerald-600' : 'text-red-600'}`}>
